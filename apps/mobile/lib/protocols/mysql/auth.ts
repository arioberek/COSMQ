...
export function createNativePasswordAuth(password: string, scramble: Buffer): Buffer {
  if (!password) {
...
export function createCachingSha2PasswordAuth(password: string, scramble: Buffer): Buffer {
  if (!password) {
    return Buffer.alloc(0);
  }
