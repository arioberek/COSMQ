...
export function buildScramClientFinal(
  ...
  const saltedPassword = pbkdf2Sha256(passwordBytes, saltBytes, iterationCount, SHA256_OUTPUT_SIZE);
  ...
  const serverSignature = bytesToBase64(hmacSha256(serverKey, utf8ToBytes(authMessage)));
...
export function verifyScramServerFinal(session: ScramSession, serverFinalMessage: string): void {
